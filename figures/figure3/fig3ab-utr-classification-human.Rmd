...
df <- read_tsv("data/utrs/hg38/utr_metadata.tsv")
...
df %<>%
  filter(expressed.tx, 
         transcript_id %in% txs_idx)
```

```{r summarize_counts_expressed}
n_txs <- df %>% distinct(transcript_id) %>% nrow
n_genes <- df %>% distinct(gene_id) %>% nrow
n_multi <- df %>% filter(utr.type.pct10.no_ipa == 'multi') %>% distinct(gene_id) %>% nrow
n_single <- df %>% filter(utr.type.pct10.no_ipa  == 'single') %>% distinct(gene_id) %>% nrow

stopifnot(n_single_all + n_multi_all == n_genes_all)
```

After processing Tabula Sapiens  to consider only for `r n_txs` transcripts
...
df_multi <- df %>%
  group_by(gene_id) %>%
  summarize(nutr_pct05=dplyr::first(utr.count.celltypes.pct05.no_ipa),
            nutr_pct10=dplyr::first(utr.count.celltypes.pct10.no_ipa),
...
ggsave("output/figure3/ed3a-n-utr-isoforms-pct05.human.pdf", width=4, height=4, dpi=300)
```
