...
### Export Table
```{r export_tbl_cells}
write_tsv(df_cells_srho, file="output/tables/tbl-sq-cr-sp-corr-umis-per-cell.tsv")
```
...
ggsave("output/figure1/fig1B-umi-corrs-pearson-inset.pdf", width=2, height=4, dpi=300)
...
ggsave("output/figure1/fig1B-umi-corrs-pearson-inset-zoom.pdf", width=2, height=4, dpi=300)
...
ggsave("output/figure1/fig1B-umi-corrs-spearman-inset.pdf", width=2, height=4, dpi=300)
...
ggsave("output/figure1/fig1B-umi-corrs-spearman-inset-zoom.pdf", width=2, height=4, dpi=300)
```
...
plot_umis_per_cell_compare <- function (sce_x, sce_y, 
  ...
  ggplot(df, aes(x=x, y=y)) +
    rasterize(geom_point(alpha=alpha, size=0.3, pch=16), dpi=300) +
...
df_counts %>%
  ...
  deframe() %>% { 
    for (id in names(.)) { 
      ...
      ## save to PDFs
      ggsave(sprintf("output/figure1/fig1B-umis-per-cell-compare-%s.pdf", id), 
...
### Export Table
```{r export_tbl_genes}
write_tsv(df_genes_srho, file="output/tables/tbl-sq-cr-sp-corr-umis-per-gene.tsv")
```
...
ggsave("output/figure1/fig1C-umi-corrs-pearson-inset.pdf", width=2, height=4, dpi=300)
...
ggsave("output/figure1/fig1C-umi-corrs-pearson-inset-zoom.pdf", width=2, height=4, dpi=300)
...
ggsave("output/figure1/fig1C-umi-corrs-spearman-inset.pdf", width=2, height=4, dpi=300)
...
ggsave("output/figure1/fig1C-umi-corrs-spearman-inset-zoom.pdf", width=2, height=4, dpi=300)
```
...
plot_umis_per_gene_compare <- function (sce_x, sce_y, 
  ...
  ggplot(df, aes(x=x+1, y=y+1)) +
    rasterize(geom_point(alpha=0.3, size=0.1, pch=16), dpi=300) +
    annotate("text", x=label_lims[1], y=label_lims[2], label=label_rho_s, size=4, parse=TRUE) +
    geom_abline(slope=1, intercept=0, linetype='dashed', color="red") +
    scale_x_log10(limits=axis_lims) + scale_y_log10(limits=axis_lims) +
    labs(x=label_x, y=label_y) +
...
df_counts %>%
  ...
  deframe() %>% { 
    for (id in names(.)) { 
      ...
      ## save to PDFs
      ggsave(sprintf("output/figure1/fig1C-umis-per-gene-compare-%s.pdf", id), 
             .[[id]], width=5, height=4, dpi=300)
    }
  }
