...
idx_row <- c(8,3,4,7,5,2,6,1)
idx_col <- c(4,3,2,9,5,7,1,6,8)
...
tbl <- df_clusters$table[[1]]
tbl[idx_row, idx_col] %>% {
  tibble(x=rep(cumsum(colSums(.)) - 0.5*sum(.[,1]), each=nrow(.)),
         y=rep(cumsum(rowSums(.)) - 0.5*sum(.[1,]), times=ncol(.)),
         height=rep(rowSums(.), times=ncol(.)),
         width=rep(colSums(.), each=nrow(.)),
         values=as.vector(./rowSums(.)))
}
  
```


```{r plot_alluvial_default, fig.width=4, fig.height=4}
df_clusters[1,] %>%
  { make_cluster_df(cr=.$clusters_10x[[1]], sq=.$clusters_sq[[1]],
                    idx_cr=c(2,3,8,6,4,7,5,1),
                    idx_sq=c(1,2,3,4,6,9,5,7,8)) } %>%
