...
TSV_TESTS="data/dge/20230516-ery-trajectory-pairwise-t-tests.tsv.gz"
...
idx_sig_genes <- rowData(sce) %>%
  { (.$gene_id %in% df_sig_genes$gene_id) }

## filter sce
sce %<>% `[`(idx_sig_genes, idx_cells)
cts_gene_cell <- assay(sce, "normcounts")

## design matrices
M_cell_celltype <- t(fac2sparse(sce$clusters))[,celltypes]

## compute luis
cts_gene_celltype <- cts_gene_cell %*% M_cell_celltype
tpm_gene_celltype <- cts_gene_celltype %>% 
...
wt_ztpm <- ztpm_gene_celltype %>% 
  { setNames(as.numeric(. %*% seq_len(ncol(.))), rownames(.)) }
...
pheatmap(log10(tpm_gene_celltype[rev(idx_utr_type),]), 
         cluster_rows=FALSE, cluster_cols=FALSE, 
         labels_col=c("HSC", "Prog1", "Prog2", "MEP", "ProE", "Baso1", "Baso2", "Ery"),
         show_rownames=FALSE, scale='none', fontsize=8,
