...
df_tests <- read_tsv("data/dtu/20211019-ery-trajectory-pairwise-lui-tests.min50.boot10k.tsv.gz", 
                     col_types='ccccddi') %>%
...
idx_lengthening <- which(names(lui_medians) %in% genes_lengthening)
idx_shortening  <- which(names(lui_medians) %in% genes_shortening)

idx_lengthening %<>% `[`(order(lui_medians[.]))
idx_shortening %<>% `[`(order(lui_medians[.]))
...
pheatmap(flui_gene_celltype[idx_ordered,], 
  ...
  save_plot(filename="output/figure4/fig4d-lui-hsc-ery-path.pdf",
