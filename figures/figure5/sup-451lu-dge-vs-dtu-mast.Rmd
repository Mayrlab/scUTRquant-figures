...
df_test_dge <- readRDS("data/dge/human.filtered.DEGs.rds")
```
...
```{r sig_dge}
df_test_dge %>%
  filter(p_val_adj < MAX_QVAL, abs(avg_log2FC) >= MIN_L2FC) %>%
...
# Export LUI Table
```{r export}
df_test_dge_final <- df_test_dge %>%
  dplyr::rename(pval=p_val, qval=p_val_adj, log2FC=avg_log2FC)

df_test_lui_final <- df_test_lui %>%
  dplyr::rename(gene_id=gene, dLUI=stat) %>%
  left_join(dplyr::select(df_test_dge_final, gene_id, log2FC, pval, qval), by="gene_id",
...
mapIds(org.Hs.eg.db, df_test_lui_final$ensembl_id, "SYMBOL", "ENSEMBL", multiVals="first") %>%
  ...
  write_xlsx("output/tables/tbl4-451Lu-resistant-lui-test-mast.bs10k.xlsx") %>%
  head(n=100) %>%
  knitr::kable()
```
