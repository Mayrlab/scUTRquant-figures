...
alternative scRNA-seq quantification tools. Both CellRanger and scUTRquant
are based on a CellRanger-filtered version of Ensembl Release 93 for GRCh38. 
...
SAMPLE_SHEET="metadata/counts_sample_sheet_pbmc_cr.csv"
...
ggsave("output/figure5/fig5A-umi-corrs-pearson-inset.pdf", width=2, height=4, dpi=300)
...
ggsave("output/figure5/fig5A-umi-corrs-pearson-inset-zoom.pdf", width=2, height=4, dpi=300)
...
ggsave("output/figure5/fig5A-umi-corrs-spearman-inset.pdf", width=2, height=4, dpi=300)
...
ggsave("output/figure5/fig5A-umi-corrs-spearman-inset-zoom.pdf", width=2, height=4, dpi=300)
```
...
df_counts %>%
  ...
  deframe() %>% { 
    for (id in names(.)) { 
      ...
      ## save to PDFs
      ggsave(sprintf("output/figure5/fig5A-umis-per-cell-compare-%s.pdf", id),
...
ggsave("output/figure5/fig5B-umi-corrs-pearson-inset.pdf", width=2, height=4, dpi=300)
...
ggsave("output/figure5/fig5B-umi-corrs-pearson-inset-zoom.pdf", width=2, height=4, dpi=300)
...
ggsave("output/figure5/fig5B-umi-corrs-spearman-inset.pdf", width=2, height=4, dpi=300)
...
ggsave("output/figure5/fig5B-umi-corrs-spearman-inset-zoom.pdf", width=2, height=4, dpi=300)
```
...
df_counts %>%
  ...
  deframe() %>% { 
    for (id in names(.)) { 
      ...
      ## save to PDFs
      ggsave(sprintf("output/figure5/fig5B-umis-per-gene-compare-%s.pdf", id),
             .[[id]], width=5, height=4, dpi=300)
    }
  }
