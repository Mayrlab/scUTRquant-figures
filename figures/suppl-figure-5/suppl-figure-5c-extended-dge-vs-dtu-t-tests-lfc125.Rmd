...
df_sig_all <- full_join(df_sig_dtu, df_sig_dge, by=c("gene_id", "start", "end")) %>%
  ...
  mutate(sig=ifelse(sig_isoform & sig_gene, "Both", 
         ...
         test_label=fct(str_c(end, " vs ", start), levels=test_lbl_lvls))
...
ggsave("output/figure4/fig4f-celltypes-dge-dtu-compared-all-ns.lfc125.pdf", width=6, height=5, dpi=300)
```
...
ggsave("output/figure4/fig4f-celltypes-dge-dtu-compared-all.lfc125.pdf", width=6, height=5, dpi=300)
```

## Plot Positive - Split
```{r plot_split, fig.width=6, fig.height=5}
...
ggsave("output/figure4/fig4f-celltypes-dge-dtu-compared-split.lfc125.pdf", width=6, height=5, dpi=300)
```
...
## Export Table
```{r export_tbl_all}
write_xlsx(df_wide_all, "output/tables/sup-tbl3-extended-celltypes-chisq-results-t-tests-all.lfc125.xlsx")
```
...
ggsave("output/figure4/fig4f-celltypes-dge-dtu-compared-multi.lfc125.pdf", width=6, height=5, dpi=300)
```
...
ggsave("output/figure4/fig4f-celltypes-dge-dtu-compared-multi-ns.lfc125.pdf", width=6, height=5, dpi=300)
```
...
## Export Table
```{r export_tbl}
write_xlsx(df_wide, "output/tables/tbl3-extended-celltypes-chisq-results-t-tests.lfc125.xlsx")
```
